...
def build_graph (json_iter, size):
    """
    ...
    for meta in json_iter:
        ...
        for pair in get_tiles(map(WordNode._make, meta["graf"]), size):
            if DEBUG:
                print(pair)
...
def text_rank (json_iter, size = 3):
    """
    run the TextRank algorithm
    """
    graph = build_graph(json_iter, size)
...
def top_keywords_sentences(text,stopwords=None, spacy_nlp=None, skip_ner=True, phrase_limit= None, sent_word_limit=150, window = 2):
    #Parse incoming text
    ...
    graph, ranks = text_rank(parse_list, window)
